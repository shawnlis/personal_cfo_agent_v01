"""Moomoo provider with guarded read-only OpenD proof support."""
...
from personal_cfo_agent.providers.moomoo_readonly_adapter import (
    ...
    MoomooFetchError,
    MoomooReadOnlyAdapter,
    MoomooSDKNotInstalledError,
)


class MoomooProvider(ProviderBase):
    ...
    def __init__(
        ...
        live_adapter: MoomooReadOnlyAdapter | None = None,
    ) -> None:
        super().__init__(config=config, allow_live_read=allow_live_read)
        self._snapshot: MoomooReadOnlySnapshot | None = None
    ...
    def _build_adapter(self) -> MoomooReadOnlyAdapter:
    ...
    def _numeric_config_is_valid(self) -> bool:
        try:
            int(self.config.settings.get("CFO_MOOMOO_PORT", ""))
        except (TypeError, ValueError):
            return False
        return True

    def _require_snapshot(self) -> MoomooReadOnlySnapshot:
