"""uSMART provider scaffold with readiness-only diagnostics."""
...
class USMARTProvider(ProviderBase):
    ...
    def validate_config(self) -> list[WarningCode]:
        diagnostics = run_usmart_connection_diagnostics(self.config.settings)
        self.diagnostics = _diagnostics_dict(diagnostics)
        return [code for code in diagnostics.warning_codes if code != WarningCode.USMART_READINESS_OK]
    ...
    def connection_diagnostics(self):
        return run_usmart_connection_diagnostics(self.config.settings)
...
def _diagnostics_dict(diagnostics) -> dict[str, object]:
    return {
        ...
        "api_key_present_redacted": diagnostics.api_key_present,
        "api_secret_present_redacted": diagnostics.api_secret_present,
