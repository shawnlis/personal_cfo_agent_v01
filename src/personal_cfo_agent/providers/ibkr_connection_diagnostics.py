...
class IBKRConnectionDiagnostics:
    ...
    session_type_redacted: str
    port_classification: str
    port_matches_session_type: bool | None
...
def run_ibkr_connection_diagnostics(
    ...
    session_type = _session_type(env.get("CFO_IBKR_SESSION_TYPE", ""))
    session_type_present = bool(session_type)
    port_classification = _port_classification(env.get("CFO_IBKR_PORT", ""))
    port_matches_session_type = _port_matches_session_type(
    ...
    warning_codes = _warning_codes(
        ...
        port_matches_session_type=port_matches_session_type,
    )
    return IBKRConnectionDiagnostics(
        ...
        session_type_redacted=session_type or "not_configured",
        port_classification=port_classification,
        port_matches_session_type=port_matches_session_type,
...
def _port_classification(value: str) -> str:
    try:
        port = int(value.strip())
    except ValueError:
        return "invalid_or_missing_port"
    return {
        4001: "gateway_live_port",
        4002: "gateway_paper_port",
        7496: "tws_live_port",
        7497: "tws_paper_port",
    }.get(port, "unknown_port_class")


def _port_matches_session_type(
    *, session_type: str, port_classification: str
) -> bool | None:
    if not session_type:
        return None
    if session_type == "gateway":
        return port_classification.startswith("gateway_")
    if session_type == "tws":
        return port_classification.startswith("tws_")
    return None
...
def _warning_codes(
    *,
    ...
    if port_matches_session_type is False:
        codes.append(WarningCode.IBKR_PORT_SESSION_TYPE_MISMATCH)
